...
#set MS2 bacteriophage as reference virus
...
create_dataset_interactions = function(df, chloride_column){
  ...
  df_virus_chars_subset <- df_virus_chars[c("virus_name_abbrev","virus_name_abbrev_orig","dsDNA","dsRNA","ssDNA","plus_ssRNA","minus_ssRNA","DNA","RNA","balt_class", "family", "genus","species", "unique_paper_count")]
...
AIC(final_model_novirus, M2_bal, M2_spe, M2_genus, M2_fam)
r.squaredGLMM(final_model_novirus)
...
nobs(final_model_novirus, M2_bal, M2_spe, M2_genus, M2_fam)
...
# Multicollinearity of pH and alpha_0  ----------------------------------------------
cor(df_encoded$alpha_0, df_encoded$pH_refpka, method = 'pearson') 

# Function to estimate inactivation rate      ----------------------------------------------
...
#View(predict_k(model = M_best_subset, df_virus_counts = df_virus_chars_counts, temp = 20, pH = 7.53, chloride = "FALSE", paper_ID = 32))
...
under_required_k <- function(model, temp_input, pH_input, chloride_input, required_CT){
  # takes a model identifier, a temperature, pH, and a chloride value, as well as the required CT at
  #those conditions, and returns the number, percent, and names or viruses that don't meet the 
  #CT standard
  ...
  list_viruses_under_req <- (subset(prediction, exponentiate(prediction$est_k) < required_k)$virus_name_abbrev_orig)
  return(list(num_under_req, percent_under_req, list_viruses_under_req))
}
...
rev_plot_initial_just_points <- function(df, model, df_virus_counts, temp, pH, chloride, paper_ID){
  ...
  df_test$virus_name_abbrev_orig <- df_test$virus_name_abbrev_orig.x
  plot <- ggplot(df_test, aes(x = log_average_kobs, y = reorder(virus_name_abbrev_orig, est_k), color = balt_class)) + 
    ...
    geom_text(x=-1.5, y=75, label="a",color = 'black', size = 8, show.legend = FALSE) +
    #geom_errorbar(aes(xmin = est_lower, xmax = est_upper))+
    #xlab(paste(temp, "C, pH", pH, " \n and", chloride_level, "chloride")) + 
    ylab("") +
    xlab(expression("Observed rate constants"~(L~mg^-1~min^-1))) +
    #annotate("text", label = "a", x = Inf, y = Inf, hjust = 30, vjust = 1.1, size = 8, fontface = "bold") +
...
rev_plot_modeled <- function(df, model, df_virus_counts, temp, pH, chloride, paper_ID){
  ...
  plot <- ggplot(df_test, aes(x = est_k, y = reorder(virus_name_abbrev_orig, est_k), color = balt_class)) + 
    geom_point() +
    geom_errorbar(data = df_test, aes(xmin = est_lower, xmax = est_upper)) +
    geom_text(x=-1, y=75, label="b",color = 'black', size = 8, show.legend = FALSE) +
    #geom_point(data = df_test, aes(x = est_k, y = reorder(virus_name_abbrev_orig, est_k)), color='black', shape = "|", size = 2) +
    scale_x_continuous(breaks=c(-3, -2, -1, 0, 1 ,2, 3),
                       labels=c("0.001","0.01","0.1", "1", "10", "100","1000")) +
    #geom_errorbar(aes(xmin = est_lower, xmax = est_upper))+
    #xlab(paste(temp, "C, pH", pH, " \n and", chloride_level, "chloride")) + 
    ylab("") +
    xlab(expression("Estimated rate constants"~(L~mg^-1~min^-1))) +
...
jpeg(file="Figure_1_June_12.jpeg",width=11.5,height=11,units="in", res=700)
just_points +just_model
dev.off()
...
plot_predictions_ssRNA_labels <- function(df_input, model, df_virus_counts, temp, pH, chloride, paper_ID){
  pred_with_CI <- predict_k(model, df = df_virus_counts, temp, pH, chloride, paper_ID)
  pred_with_CI$n <- as.numeric(pred_with_CI$n)
  df_test <- merge(x = df_input, y = pred_with_CI, by = "virus_name_abbrev", all.x = TRUE)
  df_test$balt_class <- df_test$balt_class.x
  df_test$genus <- df_test$genus.x
  
    plot <- ggplot(subset(df_test, balt_class == "+ssRNA" ), aes(x = est_k, y = reorder(virus_name_abbrev_orig.x, est_k), color = genus, shape=genus)) + 
    geom_point() +
    ...
    #theme(legend.position = c(0.8, 0.2), legend.title = element_blank())
    theme(legend.position = c(0.8, 0.2), legend.title = element_blank(), axis.ticks.y=element_blank(),axis.text.x = element_text(size = 12), axis.title=element_text(size=16,face="bold"))
...
df_virus_chars_counts
general_plot_baltclass_labels <- plot_predictions_ssRNA_labels(df_input = df_encoded, model = M_best_subset, df_virus_counts = df_virus_chars_counts, temp = 20, pH = 7.53, chloride = "FALSE", paper_ID = 32)
general_plot_baltclass_labels

jpeg(file="plot2_june_12.jpeg",width=6.5,height=6,units="in", res=2000)
general_plot_baltclass_labels
dev.off()
...
plot_predictions_five <- function(model, df_virus_counts, temp, pH, chloride, paper_ID){
  ...
  plot <- ggplot(subset(pred_with_CI, n>4 ), aes(x = est_k, y = reorder(virus_name_abbrev, est_k), color = balt_class, shape=balt_class)) + 
    ...
    ylab("") +
    xlab(expression("Estimated rate constants"~(L~mg^-1~min^-1))) +
    #geom_text(x=input_x, y=27, label=text,color = 'black', size = 8, show.legend = FALSE) +
    scale_color_manual(breaks = c("+ssRNA", "dsRNA", "dsDNA","-ssRNA","ssDNA"),
                       values=c("#75bbfd", "#fcb001", "#02ab2e",  "#7e1e9c", "#c44240")) +
    scale_shape_manual(breaks = c("+ssRNA", "dsRNA", "dsDNA","-ssRNA","ssDNA"),
                       values = c(0,1,2,4,7)) +
...
cond_1
#Figure 3b. Condition 2: Reference and high pH
...
#Figure 3c. Condition 3: Reference and low temperature 
...
#Figure 3d. Condition 3: Reference and high chloride 
...
jpeg(file="plot3_condition_jun_12.jpeg",width=6,height=6,units="in", res=600)
cond_1 + cond_2 + cond_3 + cond_4 + grid.draw(legend) + plot_layout(design = design) 
dev.off()


jpeg(file="legend.jpeg",width=5,height=1.5,units="in", res=600)
grid.newpage()
grid.draw(legend) 
dev.off()
...
legend
# Functions to return estimates of inactivation rate   ----------------------------------------------
...
under_required_k(model = M_best_subset, temp_input = 10, pH_input = 6, chloride_input = "FALSE", required_CT = 6)[3]
...
for (i in 1:length(result_list)) {
  value1 <- c(value1, result_list[[i]][[1]])
  value2 <- c(value2, result_list[[i]][[2]])
  value3[[i]] <- list(as.character(result_list[[i]][[3]]))
}

value3_concatenated <- sapply(value3, function(x) paste(x, collapse = ","))

value3

result_df <- data.frame(Value1 = value1, Value2 = value2,  Value3 = I(value3_concatenated))
result_df


#do.call(data.frame, result_list)
#result_df <- do.call(rbind.data.frame, result_list)
colnames(result_df) <- c("Number of viruses not meeting requirement", "Percent of viruses not meeting requirement","Viruses not meeting CT requirement")

result_df$viruses_not_meeting_criteria
...
write.csv(req_df, "/Users/mirac/Desktop/Chlorine_Project/chlorine_review/req_df.csv")
...
ranef(M_best_subset, condVar = TRUE)
